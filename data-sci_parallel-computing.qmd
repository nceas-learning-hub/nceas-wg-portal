...
![Levy et al. 2016. doi:10.5063/F1Z899CZ](images/data-sci_parallel-computing/levy-map.png){fig-alt="Map of North America divided into 25 equally-sized squares"}
...
![NEON Data Cube](images/data-sci_parallel-computing/DataCube.png){fig-alt="Three-dimensional screen capture of a satellite image with layers of different types of data stacked into a cube"}
...
traditional computers had a single CPU, modern computers can ship with multiple 
...
![](images/data-sci_parallel-computing/processor.png){fig-alt="Simple graphic showing a processor composed of several cores" fig-align="center"}
...
![](images/data-sci_parallel-computing/processors.png){fig-alt="Simple graphic showing several processors (composed of cores) within a larger shape labeled 'Server A'" fig-align="center"}
...
![Delta Supercomputer](https://access-ci.org/wp-content/uploads/2022/10/Delta_System.jpg){fig-alt="Photo of server hardware (computer cabinets)"}
...
![Serial and parallel execution of tasks using threads and processes.](images/data-sci_parallel-computing/serial-parallel-exec.png){fig-alt="Simple graphic showing the difference between serial, parallel threads, parallel processes, and cluster parallels"}
...
![Amdahl's Law](images/data-sci_parallel-computing/Amdahl_law.png){fig-alt="Graph showing Amdahl's Law, where the speedup of the computation is a function of both the number of cores and the proportion of the computation that can be parallelized:"}
...
![](images/data-sci_parallel-computing/penguins-logo.png){fig-alt="Hex logo for 'palmerpenguins' R package"}
...
![](images/data-sci_parallel-computing/future-logo.png){fig-alt="Hex logo for 'future' R package"}
...
The `multisession` futures plan is one in which each future task is evaluated in a separate, background R session that runs on the same host that launched the process. If the host has multiple cores available, then `multisession` will make use of these and create background sessions on each of the available cores. If all background sessions are busy, then the creation of new `future` expressions will be blocked until one is available. 

::: {layout-ncol="2"}

So, what's the point of all of this? Basically, the `future` package provides a mechanism for evaluating expressions asynchronously, which we can leverage to launch many sessions on available cores. We can harness this with another package, `furrr`, which functions as an asynchronous analogue to `purrr`. 

![](images/data-sci_parallel-computing/furrr-logo.png){width="20%" fig-alt="Hex logo for 'furrr' R package"}

:::

With `furrr`, you can use the `map()` pattern to execute an expression across a set of inputs using asynchronous futures. Most of the details here are hidden under the hood, so this usage pattern should feel really familiar to `purrr` users. For example, let's return to our bootstrap linear model, and re-implement it, first using purrr, and then with furrr.
...
- [Beyond Single-Core R](https://ljdursi.github.io/beyond-single-core-R/#/) by Jonathan Dursi (also see [GitHub repo for slide source](https://github.com/ljdursi/beyond-single-core-R))
